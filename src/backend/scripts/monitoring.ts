...
export class MonitoringService {
  ...
  private maxHistorySize = 1440; // 24h of minutes
  ...
  private evaluateAlerts(metrics: MonitoringMetrics): void {
  ...
  private calculateHealthScore(metrics: MonitoringMetrics): void {
    let score = 100;
    ...
    if (metrics.database.recentNews24h > 100) score += 5;
    if (metrics.database.recentNews7d > 1000) score += 5;
    if (metrics.database.duplicates === 0) score += 5;
    if (metrics.database.qualityIssues === 0) score += 5;
  ...
  async logToFile(metrics: MonitoringMetrics): Promise<void> {
    const logEntry = {
