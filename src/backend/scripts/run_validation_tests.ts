...
class ValidationTester {
  ...
  async testConnection(): Promise<boolean> {
    try {
      ...
      console.error(
        '❌ Connexion à la base de données: ÉCHEC',
  ...
  async testTables(): Promise<boolean> {
    try {
      const client = await this.pool.connect();
  ...
  async detectIssues(data: {
    ...
    if (data.recentNews24h < 50) {
      issues.push(
        `Peu de données récentes: seulement ${data.recentNews24h} news dans les dernières 24h`
      );
    }
    ...
    if (data.duplicates > data.totalNews * 0.1) {
      issues.push(
        `Taux de duplication élevé: ${((data.duplicates / data.totalNews) * 100).toFixed(1)}%`
      );
    }
  ...
  async runValidationTest(): Promise<TestResult> {
  ...
  printReport(result: TestResult): void {
    ...
    console.log(
      `${statusEmoji} État: ${
        result.success ? 'SUCCÈS' : result.qualityScore >= 60 ? 'CORRECTIONS NÉCESSAIRES' : 'ÉCHEC'
      }`
    );

    console.log('\n📊 MÉTRIQUES CLÉS:');
    console.log(
      `   • Base de données: ${result.databaseConnected ? 'Connectée ✅' : 'Inaccessible ❌'}`
    );
