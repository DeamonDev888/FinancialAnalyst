...
export interface CryptoPrice {
  ...
  change_24h: number;
  volume_24h: number;
  market_cap?: number; // Binance ticker doesn't give market cap directly, we might skip or estimate
}
...
export class BinanceScraper {
   ...
   */
  async fetchPrices(): Promise<CryptoPrice[]> {
    try {
      ...
      const relevantTickers = allTickers
        .filter((t: any) => this.targetSymbols.includes(t.symbol))
        .map((t: any) => ({
          symbol: t.symbol,
   ...
   */
  async saveToDatabase(pool: Pool, prices: CryptoPrice[]): Promise<void> {
