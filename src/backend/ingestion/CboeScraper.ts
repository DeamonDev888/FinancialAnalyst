...
export class CboeScraper {
  ...
  async scrapeOexRatio(): Promise<OexScrapeResult> {
    await this.init();
  ...
  async saveToDatabase(pool: Pool, result: OexScrapeResult): Promise<void> {
    if (result.put_call_ratio === null) return;

    const client = await pool.connect();
    try {
      // Create table if not exists (handled in schema update script, but good to have here conceptually)
      await client.query(
        `INSERT INTO oex_ratios (ratio, source, scraped_at) VALUES ($1, $2, NOW())`,
        [result.put_call_ratio, result.source]
      );
