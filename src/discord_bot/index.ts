...
function formatVortex500Message(result: any): string {
  return `
...
function formatVixAgentMessage(result: any): string {
...
async function postTopRelevantNewsToPrivate(): Promise<number> {
  ...
  try {
    ...
    try {
      ...
      if (news.length > 0) {
        const channel = await client.channels.fetch(PRIVATE_CHANNEL_ID) as TextChannel;
        if (!channel) {
