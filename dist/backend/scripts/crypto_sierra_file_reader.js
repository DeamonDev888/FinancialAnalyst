...
exports.CryptoFileReader = void 0;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const events_1 = require("events");
class CryptoFileReader extends events_1.EventEmitter {
    sierraDataPath;
    watchInterval = null;
    lastPrices = new Map();
     ...
     */
    extractCryptoPrice(buffer, symbol) {
        let maxPrice = 0;
        // Déterminer la plage de prix selon le symbole
        let minPrice = 0, maxExpectedPrice = 100000;
        ...
        try {
            ...
            if (cryptoData.length > 0) {
                ...
                cryptoData.forEach(data => {
                    ...
                    console.log(`${emoji} ${data.symbol} (${data.exchange}): ${data.lastPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 8 })} ${data.changePercent >= 0 ? '📈' : '📉'} ${data.changePercent >= 0 ? '+' : ''}${data.changePercent.toFixed(2)}%`);
    ...
    checkFilesStatus() {
        ...
        if (symbols.length > 0) {
            ...
            symbols.forEach(symbol => {
                ...
                console.log(`     ${symbol}: .scid ${scidExists ? '✅' : '❌'} .dly ${dlyExists ? '✅' : '❌'}`);
            });
        }
        else {
...
exports.CryptoFileReader = CryptoFileReader;
async function main() {
...
exports.default = CryptoFileReader;
if (require.main === module) {
    main();
}
//# sourceMappingURL=crypto_sierra_file_reader.js.map