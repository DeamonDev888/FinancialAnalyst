...
async function quickDatabaseAnalysis() {
    ...
    `;
        ...
        const totalNews = parseInt(statsResult.rows.find(r => r.table_name === 'news_items')?.total_rows || '0');
        const totalSentiments = parseInt(statsResult.rows.find(r => r.table_name === 'sentiment_analyses')?.total_rows || '0');
        const totalMarketData = parseInt(statsResult.rows.find(r => r.table_name === 'market_data')?.total_rows || '0');
        ...
        if (inactiveSources.length > 0) {
            console.log(`   • Vérifier les sources inactives: ${inactiveSources.map(s => s.source).join(', ')}`);
        }
        ...
        console.error("❌ Erreur lors de l'analyse:", error instanceof Error ? error.message : String(error));
    }
    finally {
        client.release();
        await pool.end();
