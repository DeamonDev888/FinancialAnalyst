...
exports.SimpleValidationTest = void 0;
...
class SimpleValidationTest {
      ...
      `);
        ...
        `);
                ...
                sourceStats.rows.forEach((row, index) => {
                    console.log(`   ${index + 1}. ${row.source}: ${parseInt(row.count).toLocaleString()} items`);
                });
        ...
        `);
                ...
                const duplicateRate = totalRecent > 0 ? ((duplicateCount / totalRecent) * 100).toFixed(1) : 0;
                if (duplicateCount > 0) {
                ...
                let score = 100;
                if (emptyCount > 0)
                    score -= Math.min(20, (emptyCount / totalRecent) * 100);
                if (invalidCount > 0)
                    score -= Math.min(20, (invalidCount / totalRecent) * 100);
    ...
    async checkTables() {
      ...
      `);
            ...
            const missingTables = expectedTables.filter(table => !tables.rows.some(row => row.table_name === table));
...
exports.SimpleValidationTest = SimpleValidationTest;
// Script principal
if (require.main === module) {
    ...
    try {
        ...
        if (!isConnected) {
            ...
            console.log('   • Les identifiants dans .env (DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD)');
...
//# sourceMappingURL=test_basic_validation.js.map