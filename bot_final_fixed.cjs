...
// Vérification et arrêt des bots existants
...
function executeScript(message, scriptName, description, emoji, isLong = false) {
  ...
  const replyMessage = await message.reply(`${emoji} ${startMessage}`);
...
- Le bot vérifie automatiquement si d\'autres instances tournent déjà
- Les bots existants sont arrêtés avant de démarrer une nouvelle instance
...
client.on('messageCreate', async (message) => {
  ...
  // Vérifier et arrêter les bots existants avant de lancer
  const existingBots = checkExistingBots();
  if (existingBots) {
    console.log('⚠️ Arrêt des bots existants avant de démarrer...');
  }
