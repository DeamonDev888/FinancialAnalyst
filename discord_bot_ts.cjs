 ...
 * Version CommonJS pour éviter les conflits d'imports/require
 */

// Charger tous les modules avec require
...
// RSS Functions - Version CommonJS
...
async function getLatestFeedsFromOpml(opmlPath, maxFeeds = 8) {
  try {
    ...
    while ((match = regex.exec(opmlData)) !== null) {
      feeds.push({
        title: match[1],
        xmlUrl: match[2],
        htmlUrl: match[3],
...
- ✅ **TypeScript** : Version corrigée et optimisée (CommonJS)
